...
export const currencyConverterState = {
  baseCurrency: 'rub',
  convertCurrency: 'usd',
  baseQuantity: 1,
  usd: 0,
...
const updateCurrencyTable = (currencies: CurrencyStateConverter) => {
  for (const currencyName in currencies) {
    const salePrice = document.querySelector(
      `.currency-table__item[data-currency=${currencyName}] > .currency-table_sale`,
    );
    const buyPrice = document.querySelector(
      `.currency-table__item[data-currency=${currencyName}] > .currency-table_buy`,
    );
...
const getCurrencyRatio = () => {
  let rightK = 1;
  let reverseK = 1;
  const convertCurr = currencyConverterState.convertCurrency;
  const baseCurr = currencyConverterState.baseCurrency;
  switch (baseCurr === convertCurr) {
    case true:
      break;
    case false:
      // eslint-disable-next-line no-case-declarations
      const basePrice = findPrice(baseCurr);
      // eslint-disable-next-line no-case-declarations
      const convertPrice = findPrice(convertCurr);
...
const updateConverterQuantuty = () => {
  ...
  if (base && result && strightKEl && reverseKEl && baseCurr && baseCurrTo && resCurr && resCurrTo) {
    ...
    const { rightK, reverseK } = getCurrencyRatio();
